...
``annual_bonus``:
    ...
    Tier 1 (first 0.75 × housing dollars of non-housing):
        + ``secondary_rate × 100 × (1000 / 3000)`` BP per dollar
...
# path. $30 of Bilt Cash (= 3,000 Bilt Cash points at 1¢ each) unlocks 1,000
# Bilt Points, so each Bilt Cash point converts to 1000/3000 = 0.333 BP.
_BILT_CASH_TO_POINTS = 1000.0 / 3000.0
...
def _bilt_cash_mode_bonus_bp(
    ...
    tier2_capacity = (
        card.accelerator_max_activations * card.accelerator_spend_limit
        if accelerator_available
        else 0.0
    )
    remaining = max(0.0, non_housing - tier1_spend)
    tier2_spend = min(remaining, tier2_capacity)
    tier3_spend = max(0.0, remaining - tier2_spend)

    # Tier 1 bonus: 4 Bilt Cash pts per non-housing dollar, each converting to
    # 0.333 BP via the housing-payment redemption.
    tier1_bonus_per_dollar = (card.secondary_currency_rate or 0.0) * 100.0 * _BILT_CASH_TO_POINTS
    tier1_bonus_bp = tier1_spend * tier1_bonus_per_dollar

    # Tier 2 bonus: accelerator adds ``bonus_multiplier`` BP per dollar on the
    # covered spend. Bilt Cash used to fund the activations is assumed to be
    # generated by the same spend (non-housing at 4%) — verified against the
    # accelerator cost budget.
...
def apply_bilt_2_housing_mode(
    ...
    * A **bilt-cash candidate** card is built by zeroing housing multipliers
    ...
    for c in cards:
        ...
        _t1, _t2, _t3, bilt_cash_bonus_bp = _bilt_cash_mode_bonus_bp(
